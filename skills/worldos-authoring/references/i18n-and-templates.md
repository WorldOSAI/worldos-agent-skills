...
- **Below the threshold:** `request_world_localization` is refused. If the creator needs other languages before then, write the overlays yourself in the world's `config.i18n[locale]` and in each installed App's `config.i18n[locale]`, following the rules above. Write them once the canonical copy is final: a later canonical copy edit below the threshold drops every overlay (authored ones included), while writes that leave canonical copy unchanged keep them.
